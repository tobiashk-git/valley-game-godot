...
static func _try_place_room(rng: RandomNumberGenerator, width: int, height: int, y_min: int, y_max: int, existing: Array, min_size: int = ROOM_MIN, max_size: int = ROOM_MAX) -> Room:
	for attempt in range(200):
		var w := rng.randi_range(min_size, max_size)
		var h := rng.randi_range(min_size, max_size)
		var x := rng.randi_range(1, width - 1 - w)
		var y := rng.randi_range(y_min, max(y_min, min(y_max, height - 1 - h)))
		...
		if ok:
			return candidate
	return Room.new(rng.randi_range(1, width - 1 - min_size), rng.randi_range(y_min, y_max), min_size, min_size)

static func _try_place_far_room(rng: RandomNumberGenerator, width: int, height: int, y_min: int, y_max: int, existing: Array, anchor: Vector2i, min_size: int, max_size: int) -> Room:
	var best: Room = null
	var best_dist := -1.0
	...
	if best:
		return best
	return _try_place_room(rng, width, height, y_min, y_max, existing, min_size, max_size)
...
static func _carve_corridor(rng: RandomNumberGenerator, map: Array, width: int, height: int, from: Vector2i, to: Vector2i) -> Array[Vector2i]:
	...
	while (x != to.x or y != to.y) and steps < max_steps:
		...
		var move_x: bool = x != to.x and (y == to.y or rng.randf() < 0.5)
		if move_x:
			x += signi(to.x - x)
		else:
			y += signi(to.y - y)
		...
		cells.append(Vector2i(x, y))
		if rng.randf() < 0.15:
			x = clampi(x + (-1 if rng.randf() < 0.5 else 1), 1, width - 2)
		steps += 1
	map[to.y][to.x] = FLOOR
...
# `seed_value` >= 0 makes the layout reproducible (a dungeon keeps its
# shape for the whole save - GameState.dungeon_seed()); -1 = random.
static func generate(width: int, height: int, seed_value: int = -1) -> Dictionary:
	...
	var entrance_room := _try_place_room(rng, width, height, int(height * 0.65), height - 1 - ROOM_MIN, [])
	...
	for i in range(ROOM_COUNT - 2):
		var r := _try_place_room(rng, width, height, 1, height - 1 - ROOM_MIN, rooms)
		rooms.append(r)
		intermediate_rooms.append(r)

	var boss_room := _try_place_far_room(rng, width, height, 1, height - 1 - FAR_ROOM_MIN, rooms, entrance_center, FAR_ROOM_MIN, FAR_ROOM_MAX)
	...
	var door_y := height - 1
	_carve_corridor(rng, map, width, height, entrance_center, Vector2i(door_x, door_y - 1))
	...
	for room in intermediate_rooms:
		...
		corridors.append(_carve_corridor(rng, map, width, height, prev_center, c))
		prev_center = c
	corridors.append(_carve_corridor(rng, map, width, height, prev_center, boss_room.center()))
