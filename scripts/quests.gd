...
const QUEST_DEFS := {
	...
	"open_ancient_barrow": {
		...
		"objective": {"type": "gather", "item_id": "stone", "amount": 6, "goal": "Bring the Elder 6 Stone to shore up the barrow's collapsed entrance. Forge the leather set at the Blacksmith's bench while you gather."},
		"reward": {"xp": 90, "gold": 40, "item_id": "healing_potion", "item_amount": 1},
		"dialogue": {
			"offer": "There's an old barrow at the edge of the valley - sealed for as long as anyone remembers. Bring me 6 Stone to shore up its collapsed entrance and I'll show you where it lies. And traveler: forge yourself a leather set at the Blacksmith's bench before you go down - wood and stone from the valley make every piece. All five of them.",
			"in_progress": "Still need 6 Stone for the barrow's entrance. And forge that leather at the Blacksmith's bench - armour, cap, greaves, boots, gloves. All of it, from wood and stone.",
	...
	"hunt_barrow": {
		...
		"objective": {"type": "defeat_bosses", "boss_ids": ["golden_plains_boss"], "label": "Barrow Warden asleep", "goal": "Go down into the Ancient Barrow, learn its ways, and put the Barrow Warden to sleep. Wear the whole leather set (forged at the Blacksmith's bench)."},
		"reward": {"xp": 100, "gold": 40, "item_id": "healing_potion", "item_amount": 1},
		"dialogue": {
			"offer": "The barrow's open. Something stirs at the bottom of it - the Barrow Warden, the old stories call it. Go down, find your feet, and put it to sleep. Full leather, mind: armour, cap, greaves, boots and gloves. The Blacksmith's bench makes them all from wood and stone.",
			"in_progress": "The Barrow Warden still stirs. If it keeps throwing you out, you're missing a piece of leather - the Blacksmith's bench forges the lot.",
	...
	"hunt_frostpeak": {
		...
		"objective": {"type": "defeat_bosses", "boss_ids": ["frostpeak_boss"], "label": "Glacial Revenant asleep", "goal": "The ice caves lie north-east of the ford, through the crystal passage the stone sentinels guard. Put the Glacial Revenant to sleep, then tell the Village Elder."},
		"reward": {"xp": 180, "gold": 50, "item_id": "healing_potion", "item_amount": 1},
		"dialogue": {
			"offer": "The ford holds and you've a hound at your side - good. Something haunts the ice caves up the ridge; the old hands call it the Glacial Revenant. Head north-east from here and follow the crystal passage - the stone sentinels along it are the sign you're on the right road. Put it to sleep, then tell the Elder in the village - he'll want to hear it from you.",
	...
	"hunt_verdantwood": {
		...
		"objective": {"type": "defeat_bosses", "boss_ids": ["verdantwood_boss"], "label": "Elder Bramblewood asleep", "goal": "The grove lies south-east of the ford, along the avenue of mighty oaks where the forest spirits drift. Put Elder Bramblewood to sleep, then tell the Village Elder."},
		"reward": {"xp": 220, "gold": 60, "item_id": "healing_potion", "item_amount": 1},
		"dialogue": {
			"offer": "The crossing's clear and Eden's with you. Something old and thorned sits at the heart of the wood - Elder Bramblewood. Bear south-east from here and follow the great oaks; where the forest spirits drift thickest, the grove is near. Put it to sleep, then tell the Elder in the village; he'll want to know the forest breathes again.",
	...
	"hunt_badlands": {
		...
		"objective": {"type": "defeat_bosses", "boss_ids": ["badlands_boss"], "label": "Cinderjaw asleep", "goal": "The caldera lies south-east of the ford, past the fields of fire geysers the ash golems wade through. Put Cinderjaw to sleep, then tell the Village Elder."},
		"reward": {"xp": 260, "gold": 70, "item_id": "healing_potion", "item_amount": 1},
		"dialogue": {
			"offer": "Crossing holds, and you've the pair of them with you - good. Something's been setting the badlands alight from the caldera; I call it Cinderjaw. Keep south-east through the geyser fields - mind the vents, and the ash golems that wade through them - and you'll come to the caldera. Put it to sleep before the fires spread, then tell the Elder - he'll want the news.",
	...
	"hunt_gloomfen": {
		...
		"objective": {"type": "defeat_bosses", "boss_ids": ["gloomfen_boss"], "label": "The Bogmaw asleep", "goal": "The sunken temple lies south-west of the ford, deep among the swamp trees where the spectral undead gather thickest. Put the Bogmaw to sleep, then tell the Village Elder."},
		"reward": {"xp": 300, "gold": 80, "item_id": "healing_potion", "item_amount": 1},
		"dialogue": {
			"offer": "The boardwalk holds. The marsh has a mouth at the end of it - I call it the Bogmaw, and it's the worst thing in this valley. Bear south-west through the swamp trees; the dead walk thickest around the sunken temple, so where they crowd, you're close. Put it to sleep, then tell the Elder; he's been waiting on that news longer than any of us.",
