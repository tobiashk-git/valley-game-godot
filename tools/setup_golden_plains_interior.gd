extends SceneTree
# Builds the Golden Plains terrain TileSet (wall/floor only - no hazard
# sources, this is the simplest biome interior yet) and
# GoldenPlainsInterior.tscn (using scripts/golden_plains_interior.gd, which
...
# Run via: godot --headless --script res://tools/setup_golden_plains_interior.gd

const FULL_TILE_COLLISION: PackedVector2Array = [Vector2(-16, -16), Vector2(16, -16), Vector2(16, 16), Vector2(-16, 16)]

const TEXTURES := {
	"res://assets/golden_plains_wall.png": Color(0.35, 0.28, 0.18),
	"res://assets/golden_plains_floor.png": Color(0.55, 0.45, 0.28),
}
...
func _initialize() -> void:
	print("=== Golden Plains interior setup starting ===")
	...
	_add_source(terrain_tileset, "res://assets/golden_plains_wall.png", 0, true)   # SRC_WALL
	_add_source(terrain_tileset, "res://assets/golden_plains_floor.png", 1, false) # SRC_FLOOR
	ResourceSaver.save(terrain_tileset, "res://resources/golden_plains_terrain_tileset.tres")
	...
	var root := Node2D.new()
	root.name = "GoldenPlainsInterior"
	root.set_script(load("res://scripts/golden_plains_interior.gd"))
	root.boss_id = "golden_plains_boss"
	root.entrance_tile = World.GOLDEN_PLAINS_INTERIOR_ENTRANCE
	root.poi_id = "golden_plains_interior"
	...
	packed.pack(root)
	var err := ResourceSaver.save(packed, "res://scenes/GoldenPlainsInterior.tscn")
	print("GoldenPlainsInterior.tscn saved: ", err)

	print("=== Golden Plains interior setup complete ===")
	quit()
