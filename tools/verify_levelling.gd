...
func _initialize() -> void:
	...
	print("A Dungeon Rat (12 HP, ATK 3, DEF 0) is worth 9 XP; a Skeleton (18/5/2) 18; a boss pays double: ", enemies.xp_for(rat) == 9 and enemies.xp_for(enemies.ENEMIES.skeleton) == 18 and enemies.xp_for(enemies.BOSSES.dungeon_boss, true) == 2 * enemies.xp_for(enemies.BOSSES.dungeon_boss))
	...
	for line in combat.battle_log:
		if line.contains("+9 XP."):
			xp_line = true
		if line.begins_with("Level up! Oliver is now level 2 (+4 HP, +2 MP, +1 STR, +1 AGI)."):
			level_line = true
	print("Winning logs '+9 XP.' on the doze-off line and the level-up with its gains: ", xp_line and level_line and character.stats.level == 2 and character.stats.xp == 4)
	var popup := false
	...
	print("Snapshot carries level and xp: ", int(snap.character.stats.level) == 2 and int(snap.character.stats.xp) == 4)
	character.stats.level = 9
	character.stats.xp = 99
	save.apply(snap)
	print("Applying it restores level 2, 4 XP: ", character.stats.level == 2 and character.stats.xp == 4)
