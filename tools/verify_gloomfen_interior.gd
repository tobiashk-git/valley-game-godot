...
func _initialize() -> void:
	...
	quests.quest_state["hunt_badlands"] = "completed" # the story runs in order (2026-09-08): this ford needs the previous chapter's hunt
	...
	character.stats.max_hp = 2000 # the Bogmaw hits ~29 a round since the set-rule pass; 500 fell in 17 rounds
	...
	game_state.boss_defeated.gloomfen_boss = false # monsters only roam while the boss is awake - wake it for the pool check
	...
	game_state.boss_defeated.gloomfen_boss = true
	if got_encounter:
