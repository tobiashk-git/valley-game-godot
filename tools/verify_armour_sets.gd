...
func _initialize() -> void:
	...
	print("Seven slots in the table and the equipment dict (weapon, armor, accessory, head, legs, feet, hands): ", character.SLOTS.size() == 7 and character.SLOTS.has("head") and character.SLOTS.has("legs") and character.SLOTS.has("feet") and character.SLOTS.has("hands") and character.equipment.size() == 7 and character.SLOTS.hands.stat == "defense")
	var all_ok := true
	for id in PIECES:
		...
		if def.is_empty() or not (def.get("slot", "") in ["head", "legs", "feet", "hands"]) or int(def.get("defense", 0)) < 1 or tex == null or tex.get_size() != Vector2(576, 256) or not icon_ok:
			all_ok = false
			print("  bad piece: ", id)
	print("All 19 pieces have a head/legs/feet/hands slot, defence, a 576x256 layer and an icon: ", all_ok)
	...
	print("Set bonuses per tier with gloves: frost +6, ironwood +9, ember +10, bog-iron +14 (the simulator's profiles): ", totals.frost == 6 and totals.ironwood == 9 and totals.ember == 10 and totals.bogiron == 14)
	var recipes_ok := true
	for id in PIECES:
		if not crafting.RECIPES.has(id) or crafting.RECIPES[id].result != id:
			recipes_ok = false
	print("Every piece - the leather set included - has a recipe; nothing wearable is for sale anywhere: ", recipes_ok and shop.stock_for("village_blacksmith").is_empty() and not shop.stock_for("village_trader").any(func(id: String) -> bool: return items.is_equippable(id) and items.ITEMS[id].get("slot", "") != "accessory"))
	...
	character.equip("hands", "bogiron_gloves")
	await process_frame
	print("Full bog-iron set: defence 11 + 4 + 4 + 3 + 3 = 25 from gear_total: ", character.gear_total("defense") == 25 and root.get_node("Combat")._player_defense_bonus() == 25)
	...
	for slot in BOGIRON_LAYER.keys():
		...
		if not l.visible or not l.get_meta("layer_path", "").ends_with(BOGIRON_LAYER[slot] + ".png"):
			all_visible = false
	print("Walking sprite: layers drawn legs, feet, body, gloves, head, all showing the bog-iron pieces: ", order == ["LegsLayer", "FeetLayer", "ArmorLayer", "HandsLayer", "HeadLayer"] and all_visible)
	...
	save.apply(snap)
	print("A save carries the new slots (helm, boots and gloves back after apply): ", character.equipped_id("head") == "bogiron_helm" and character.equipped_id("feet") == "bogiron_boots" and character.equipped_id("legs") == "bogiron_greaves" and character.equipped_id("hands") == "bogiron_gloves")
	...
	for slot in character.SLOTS:
		...
		if not win_n.encloses(r) or r.size != Vector2(44, 44):
			row_ok = false
	print("Phone: seven 44px header slots in one row inside the window: ", layout.is_narrow() and row_ok)
	...
	print("Leather greaves complete the leather set: legs slot, DEF 1, forged from 3 wood and 2 stone, with a walking layer and an icon: ", items.ITEMS.leather_greaves.slot == "legs" and items.ITEMS.leather_greaves.defense == 1 and crafting.RECIPES.leather_greaves.cost == {"wood": 3, "stone": 2} and ResourceLoader.exists(items.ITEMS.leather_greaves.layer) and ResourceLoader.exists("res://assets/icons/leather_greaves.png"))
	quit()
