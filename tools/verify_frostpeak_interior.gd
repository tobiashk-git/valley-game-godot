...
func _initialize() -> void:
	...
	quests.quest_state["hunt_dungeon"] = "completed" # the story runs in order (2026-09-08): this ford needs the previous chapter's hunt
	...
	game_state.boss_defeated.frostpeak_boss = false # monsters only roam while the boss is awake - wake it for the pool check
	...
	game_state.boss_defeated.frostpeak_boss = true
	if got_encounter:
	...
	var fp_row: Button = null
	for row in map_view.places_list.get_children():
		if row is Button and row.visible and row.text.strip_edges() == "Frostpeak Ice Caves":
			fp_row = row
