...
func _initialize() -> void:
	...
	var altar: Sprite2D = ow.get_node_or_null("YSort/AltarSprite")
	print("The altar prop (Leonardo, 40x77) stands in the YSort with its foot on the altar tile's bottom edge: ", altar != null and altar.texture.get_size() == Vector2(40, 77) and absf(altar.position.x - (world.ALTAR_POS.x * 32 + 16)) < 1.0 and altar.position.y == world.ALTAR_POS.y * 32 + 32 and altar.offset.y == -altar.texture.get_height() / 2.0)
