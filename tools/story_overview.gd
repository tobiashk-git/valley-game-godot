...
const WHERE := {
	...
	"Frostpeak Ranger": "camp by the northern ford",
	"Forest Druid": "glade by the eastern ford",
...
const UNLOCKS := {
	"meet_villagers": "the village gates open - the valley (Golden Plains), the dungeon gate and the castle become reachable",
	"gather_wood": "nothing in the world - the Elder's first errand (potion reward)",
	...
	"two_guardians": "two Magic Crystals at the altar reveal the Ancient Warden's lair at the valley's edge",
	"ancient_warden": "the Warden's crystal at the altar opens the portal to a second world (world 2, not built out)",
	"open_ancient_barrow": "the Ancient Barrow entrance appears in the Golden Plains (its own interior and boss)",
...
func _initialize() -> void:
	...
	_out(lines, "")
	_out(lines, "Reading it: a quest is OFFERED only when everything in its Gate is turned in (Quests.is_available). A chapter's quests form a chain (ford -> joining event -> hunt). The four biome chapters can be played in any order today - only Meet the Village gates them all.")
	_out(lines, "")
	...
	_out(lines, "- The dungeon and the castle open with the village gates; nothing sends Oliver there before The Two Guardians (chapter 6).")
	_out(lines, "- The four biome chapters are gated only by Meet the Village, so a player can take them in any order (kept open for testing).")
