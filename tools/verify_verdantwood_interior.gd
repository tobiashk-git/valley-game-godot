...
func _initialize() -> void:
	...
	print("Ford walkable LIVE in this same scene, no reload needed: ", terrain0.get_cell_source_id(world.BIOME_FORDS[world.Zone.VERDANTWOOD]) == world.SRC_FORD)
	...
	game_state.boss_defeated.verdantwood_boss = false # monsters only roam while the boss is awake - wake it for the pool check
