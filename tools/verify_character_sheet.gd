...
func _initialize() -> void:
	...
	print("Header slot row sits under the bars (44px slots, seven inside the window): ", slot_rect.position.y > sheet.mp_bar.get_global_rect().end.y and slot_rect.size == Vector2(44, 44) and sheet.get_node("Window/Header/HandsSlot").get_global_rect().end.x <= sheet.window.get_global_rect().end.x)
