extends SceneTree
# Toolbar (PanelButtons) verification. Inventory and Character are tabs of
# the CharacterSheet window since UI redesign Phase 1; Crafting/Journal/Map
...
func _initialize() -> void:
	...
	panel_buttons.inventory_btn.pressed.emit()
	await process_frame
	print("Switching to Inventory closes Crafting: ", sheet.is_open() and not crafting_panel.get_node("Panel").visible)

	# --- The other sheet tab's button switches tabs rather than closing. ---
	panel_buttons.character_btn.pressed.emit()
	await process_frame
