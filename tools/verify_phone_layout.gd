...
func _initialize() -> void:
	...
	print("Quick bar is a 2x2 block over the E button (centred on it, inside the width, above it): ", quick_bar.hbox.columns == 2 and absf(bar_rect.get_center().x - (interact_rect.position.x + 40.0)) < 2.0 and bar_rect.size.y > 60.0 and bar_rect.position.x >= 0.0 and bar_rect.end.x <= 400.0 and bar_rect.end.y <= interact_rect.position.y, " bar=", bar_rect)
	...
	print("Short phone, Map: map steps down to 2px/tile so the pane fits: ", mv2.base_scale == 2.0 and mv2.detail_pane.get_global_rect().end.y <= win_bottom and mv2.travel_btn.get_global_rect().end.y <= win_bottom)
	sheet.open("character")
	await process_frame
	await process_frame
	var cv: Control = sheet.character_view
	print("Short phone, Hero: scroll extent covers the whole stats column (stats only - no Game block): ", cv.custom_minimum_size.y >= sheet.stats_list.position.y + sheet.stats_list.size.y and sheet.stats_list.get_node_or_null("QuitBtn") == null and sheet.stats_list.get_child(0).text.begins_with("Level"))
	sheet.open("journal")
	await process_frame
